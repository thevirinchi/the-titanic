<?php
	...
	$_SESSION['fatName'] = $dbFatName;
	$_SESSION['motName'] = $dbMotName;
	$_SESSION['course'] = $dbCourse;
	$_SESSION['branch'] = $dbBranch;
	$_SESSION['aadhar'] = $dbAadhar;
	$sql = "SELECT * FROM studdets WHERE rollNum=" . $_SESSION['id'] . ";";
	$result = mysqli_query($con,$sql);
	$row = mysqli_fetch_array($result);
	$_SESSION['sem'] = $sem = $row['sem'];
	$_SESSION['year'] = $year = $row['year'];
	$_SESSION['sgpa'] = $sgpa = $row['sgpa'];
	$_SESSION['cgpa'] = $cgpa = $row['cgpa'];
	//$_SESSION['FSem'] = $dbSem;
			...
			<DIV class="row-padding margin-bottom">
				<DIV class="quarter">
					<DIV class="container red padding-16">
						<DIV class="left"></DIV>
					        <DIV class="right">
							<?php
								echo "<H3>".$_SESSION['sem']."<SUP>rd</SUP></H3>";
							?>
						</DIV>
					        <DIV class="clear"></DIV>
					        <H4>Semester</H4>
				...
				<DIV class="quarter">
					<DIV class="container blue padding-16">
						...
						<DIV class="right">
							<?php
								echo "<H3>".$_SESSION['year']."<SUP>nd</SUP></H3>";
							?>
						</DIV>
					        <DIV class="clear"></DIV>
					        <H4>Year</H4>
          ...
          <TR>
            ...
            <TH> Marks </TH>
				...
				<?php
					...
					$sql = "SELECT * FROM grades WHERE sid=".$_SESSION['id'].";";
					$result = mysqli_query($con,$sql);
					foreach($result as $row){
						$sub = $row['subid'];
						$mar = $row['marx'];
            echo "<TR><TD>".$sub."</TD><TD>".$mar."</TD></TR>";
          }
				?>
      </TABLE>
			</DIV>
